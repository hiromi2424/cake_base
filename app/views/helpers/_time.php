 ...
 */
	function nice($dateString = null, $userOffset = null) {
		...
		$time = 'H時';
		if(date("s", $date) != '00'){
			$time .= __('i分',true) . __('s秒',true);
		}elseif(date("i", $date) != '00'){
			$time .= __('i分',true);
		}
		...
		$weekDayName = $weekDayNames[date('w',$date)];

		$ret = date(__("Y年n月j日($weekDayName) $time",true), $date);
 ...
 */
	function niceShort($dateString = null, $userOffset = null) {
		...
		$y = $this->isThisYear($date)  ? '' : __('Y年',true);
		$m = $this->isThisMonth($date) ? '' : __('n月',true);
		...
		if ($this->isToday($date)) {
			$ret = sprintf(__('今日の%s',true), $time );
		} elseif ($this->wasYesterday($date)) {
			$ret = sprintf(__('昨日の%s',true), $time );
		} elseif ($this->isTomorrow($date)) {
			$ret = sprintf(__('明日の%s',true), $time );
		} else {
			$ret = sprintf(__("{$y}{$m}j日 %s",true),$time );
		}

		return $this->output(date($ret,$date));
	}
/**
